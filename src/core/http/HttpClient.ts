import { HttpRequestConfig, HttpResponse, IHttpInterceptor } from './HttpInterceptor';
import { RetryPolicy, DefaultRetryPolicy, RetryPolicyHelper } from './RetryPolicy';
...
export class HttpClient {
     ...
     * POST request
     */
    async post<T>(url: string, data?: any, config?: Partial<HttpRequestConfig>): Promise<T> {
        return this.request<T>({
            url,
            method: 'POST',
            body: data,
     ...
     * PUT request
     */
    async put<T>(url: string, data?: any, config?: Partial<HttpRequestConfig>): Promise<T> {
        return this.request<T>({
            url,
            method: 'PUT',
            body: data,
     ...
     * PATCH request
     */
    async patch<T>(url: string, data?: any, config?: Partial<HttpRequestConfig>): Promise<T> {
        return this.request<T>({
            url,
            method: 'PATCH',
            body: data,
    ...
    private async request<T>(requestConfig: HttpRequestConfig): Promise<T> {
        ...
        const retryPolicy = fullConfig.retryPolicy || this.config.retryPolicy || DefaultRetryPolicy;
        ...
        for (let attempt = 1; attempt <= retryPolicy.maxAttempts; attempt++) {
            try {
                ...
                const delay = RetryPolicyHelper.getRetryDelay(error, attempt, retryPolicy);
                this.logger.warn(`Request failed, retrying (${attempt}/${retryPolicy.maxAttempts})`, {
                    url: fullConfig.url,
                    delay
                });
