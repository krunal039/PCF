...
export class iHubService {
  ...
  async executeAction(
    ...
    try {
      ...
      const response = await this.httpClient.post<iHubResponse>(
        endpoint,
        parameters || {}
      );
  ...
  private buildQueryString(options?: iHubQueryOptions): string {
    ...
    if (options.filters) {
      queryParams.push(
        `filters=${encodeURIComponent(JSON.stringify(options.filters))}`
      );
    }
